...
class DB_Mongo ():
    ...
    def insert_to_db(self, collection:str, data: dict|list[dict]):
        if not self.is_available: return

        collection: Collection = self.db.get_collection(collection)
    ...
    def replace_to_db(self, path:str, query:dict[str, Any], data: dict[str, Any]|list):
        if not self.is_available: return

        collection: Collection = self.db.get_collection(path)
        if type(data) is list:
            data = self._doc_b64_to_obj_id(data)
    ...
    def update_to_db(self, path:str, query:dict[str, Any], data: dict[str, Any], update_many: bool = False):
        if not self.is_available: return
    ...
    def delete_to_db(self, path:str, query:dict[str, Any], delete_many: bool = False):
        if not self.is_available: return
    ...
    def get_from_db(self, path:str, query: dict[str, Any]={}):
        if not self.is_available: return
